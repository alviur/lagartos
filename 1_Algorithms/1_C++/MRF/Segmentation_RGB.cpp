...
int main(int argc, char** argv)
{
   ...
   char *outfilenameResultadoMRF  = argv[3];//Tercer argumento despues del nombre del programa sera el nombre del resultado del MRF
    ...
    img_srcBinary=cv::imread(infilenameColorMask,0);
...
int const max_kernel_size = 21;
 ...
 Mat element = getStructuringElement( 2,
                                       Size( 2*erosion_elem + 1, 2*erosion_elem+1 ),
...
//PARAMETRO BASADO EN MASCARA DE COLOR
	...
	if( img_srcBinary.at<uchar>(i,j)>20)
	{
	 img_srcBinary.at<uchar>(i,j)=1;
   ...
   Model gm(SpaceType(numbersOfLabels.begin(), numbersOfLabels.end()));
	...
	for(int i=0;i<N;i++)
	{
		...
		f(1) =(img_srcGray.at<uchar>(i,j))*img_srcBinary.at<uchar>(i,j);
//f(1) =(img_srcBinary.at<uchar>(i,j))* img_src_posterizada.at<uchar>(i,j)*img_srcRGB.at<cv::Vec3b>(i,j)[1];
		f(0) =(250-img_srcGray.at<uchar>(i,j));//*(1-posterizada.at<uchar>(i,j));
   ...
   // add 2nd order functions for all variables neighbored on the grid
	...
	//AbsoluteDifferenceFunction Absolute(numLabel, numLabel,0.5);
      ...
      PottsFunction potts(numLabel, numLabel, 9,5);
